...
\begin{document}	
	...
	A successful failure pattern recognition system can go a long way in improving spare parts management in terms of allowing a good maintenance plan. Fault detection manifests itself as a part of condition based maintenance (CBM) \cite{McKee2014}. Failure pattern recognition maybe done not only to differentiate between failure and normal operation signals, but also to distinguish between different failure patterns. Identifying patterns leading to failure opens up possibilities for early failure detection and also investigating the root cause of failure. Meanwhile, distinguishing different failure patterns may lead to effective fault management action plans, as failures can then be accurately located. These opportunities lead to efforts on automatic failure pattern recognition, one of which can be done by applying classification algorithms. 
	...
	The rest of this paper is organized as follows. Section 1 describes the task at hand by laying out the research methodology and describing the data to be worked on. In Section 2, data preparation steps which involves preprocessing the raw data, feature definitions, and initial exploratory analysis are carried out. The analysis work starts with data balancing experiments in Section 3, which covers both binary and multi class datasets. Section 4 provides a step-by-step SVM model building process, ranging from choice of kernel up to parameter optimization. Additionally, dimensionality reduction and feature selection steps are also included in this section. Finally, Section 5 sums up the results of the overall analysis work, while mentioning the possibilities for future researches.   
...
%TODO: [TABLE SAMPLE.1]
A total of nine binary datasets are included in this initial resampling implementation experiment. Each dataset is a combination of one of the three failure types and one of the three normal types. The number of majority and minority instances along with the majority to minority ratio for each dataset are presented in table [TABLE SAMPLE.1]. All nine binary sampling methods described in the previous subsections are applied to these datasets. Furthermore, every method and dataset combination is repeated ten times, yielding 90 cross validation error rates for each sampling method.

\item[Multi Class Dataset]
%TODO: [EQUATION SAMPLE.1]
Three failure types and three normal types are all put together into a unified dataset. This 6-class dataset is then subjected to four different training setups: one is directly using the dataset for training and the other three are balancing techniques. These balancing techniques are the two multi-class sampling methods, SMean and SMedian with random over- and under-sampling, and a simple class weighting scheme. For this weighting scheme each class is weighted according to its inverse proportion, which are then normalized so that all weights sum up to one, see equation [EQUATION SAMPLE.1]. Each of these four SVM training setups is repeated a hundred times. One hundred cross validation error rates for every setups are used for comparison.
\end{description}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{ | l | l | l | l | l | l | l | l | l | l |}
			...
			\multicolumn{1}{ | c | }{\textbf{Method}} 
				& \multicolumn{1}{ | c | }{\textbf{ros}}
				& \multicolumn{1}{ | c | }{\textbf{smo50}}
				& \multicolumn{1}{ | c | }{\textbf{smoD}}
				& \multicolumn{1}{ | c | }{\textbf{rus}}
				& \multicolumn{1}{ | c | }{\textbf{oss}}
				& \multicolumn{1}{ | c | }{\textbf{cnn}}
				& \multicolumn{1}{ | c | }{\textbf{enn}}
				& \multicolumn{1}{ | c | }{\textbf{ncl}} 	
				& \multicolumn{1}{ | c | }{\textbf{tom}}\\
			\hline
			\textbf{\textit{p}-value} & $1.4*10^{-12}$ & 0.009 & $6.6*10^{-11}$ & 0.318 & $3.3*10^{-5}$ & 0.048 & 0.999 & 0.594 & 0.697\\
			\hline
		\end{tabular}
		\caption{Reported p-value from Nemenyi test for the performances of nine sampling methods compared to training on the original binary datasets.} 
...
\begin{description}
\item[Binary Datasets] Table \ref{table:binarytest} suggests that all three oversampling based methods, the random oversampling and two SMOTE variants, performed significantly better that training without data balancing. However, Figure \ref{fig:binarybox} provides further insight about the cross validation error rate distributions between those three sampling methods. Both random oversampling and the default SMOTE implementation (smoD) seem to have minimal overlap with training results on imbalanced datasets. Furthermore, the SMOTE variant which enforces 50:50 ratio and majority class shrinkage (smo50) appears to vary more. A suggested cause here is that in 'smoD' there are cases where the majority class are actually randomly oversampled instead of undersampled, this may alleviate information loss from the undersampling step of SMOTE. The major take-away here is that in this particular case of data imbalance inflating the size of minority classes, which are the failure patterns, may lead to significance improvement of SVM performance in terms of having low error rate. Undersampling, on the other hand, seems to cause unnecessary information loss of normal patterns.   
